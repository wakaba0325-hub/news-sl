...
- 同名複数がヒットする場合は以下を順に試して一意化:
    ...
    2. 法人格を含む商号がそのまま完全一致する候補が1件だけならそれを採用
       (例: "吉本興業株式会社"はcore_key「吉本興業」だけで10件ヒットしあいまいになるが、
       うち商号が一字一句"吉本興業株式会社"と完全一致するのは1件だけなのでそれに決め打ちできる。
       2026-07-12の実データ検証でambiguous38件中8件をこれで解決できた)
    3. なお複数残る場合、`body_snippet`からプレスリリース冒頭でよくある
       "(本社：東京都渋谷区、...)"のような自己申告の所在地表記を都道府県名で抽出し、
       本店所在都道府県が一致する候補に絞り込み(求人のlocationに相当する信号が
       ニュース記事には無いため、本文の自己紹介文から代用)
    4. 完全未マッチの場合のみ、注記・拠点表記・「市役所/役場/庁」を除去して再突合
       (例: "狭山市役所"はcompany_masterに存在しないが、末尾の「役所」を除いた
       "狭山市"であれば存在する)
...
_GOV_SUFFIX_RE = re.compile(r"(役所|役場|庁)$")
...
def index_company_master(
    ...
    """company_master行(iterable of dict) → {core_key: [(法人番号, 都道府県, 処理区分, 商号), ...]}。"""
    idx: dict = {}
    for r in cm_rows:
        h = (r.get(houjin_col) or "").strip()
        if not h:
            continue
        shogo = r.get(shogo_col, "")
        key = core_key(shogo)
        if not key:
            continue
        idx.setdefault(key, []).append(
            (
                h,
                (r.get(pref_col) or "").strip(),
                (r.get(shobun_col) or "").strip(),
                (shogo or "").strip(),
            )
        )
    return idx
...
def _resolve(
    cands: list[tuple[str, str, str, str]], location_text: str, raw_name: str = ""
):
    ...
    if raw_name:
        exact = sorted({c[0] for c in pool if c[3] == raw_name})
        if len(exact) == 1:
            return exact[0], False, "shogo_exact"
...
def match_houjin(company_name: str, index: dict, location_text: str = ""):
    ...
    cands = index.get(key)
    if cands:
        return _resolve(cands, location_text, raw_name)
    ...
    if note_key and note_key != key:
        ...
        if cands:
            houjin, ambiguous, method = _resolve(cands, location_text, raw_name)
    ...
    gov_key = core_key(_GOV_SUFFIX_RE.sub("", raw_name))
    if gov_key and gov_key != key:
        cands = index.get(gov_key)
        if cands:
            houjin, ambiguous, method = _resolve(cands, location_text, raw_name)
            return (
                houjin,
                ambiguous,
                ("gov_suffix_stripped" if method == "exact" else method),
            )
