...
- 同名複数がヒットする場合は以下を順に試して一意化:
  ...
  2. 法人格を含む商号がそのまま完全一致する候補が1件だけならそれを採用
     (`shogo_exact`。例:「吉本興業株式会社」はcore_key正規化だけだと10件ヒットするが、
     商号が一字一句完全一致するのは1件のみ)
  3. なお複数残れば、`body_snippet`(プレスリリース冒頭の「本社：東京都〜」等の
     自己紹介文)から都道府県を抽出し本店所在都道府県が一致する候補に絞り込み
     (求人のlocationに相当する構造化フィールドがニュースには無いため代用)
  4. 完全未マッチの場合のみ、注記・拠点表記・「市役所/役場/庁」を除去して再突合
     (`gov_suffix_stripped`。例:「狭山市役所」→「狭山市」)
  - 解決できなければ `is_ambiguous_company=1`(法人番号は空)のまま

2026-07-12の実データ検証(182件)でマッチ率67.0%→72.0%に改善(shogo_exact/gov_suffix_stripped追加)。
